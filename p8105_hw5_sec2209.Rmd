...
In the following code chunk, I create a simulation to explore the effects of power on a simple linear regression. For this simulation, I set the sample size to 30 and standard deviation to 5. I modify the sample mean in 1-unit increments from 0 to 6, running 5,000 simulated data sets per mean. 
...
sim_results = 
  ...
  mutate(
    output_lists = map(.x = mu, ~ rerun(500, sim_mean_sd(mu = .x))),
...
sim_results_summary %>%
  ...
  geom_point(size = 3) +
  labs(
    title = "Power by the true value of mu",
    x = "True Mean",
    y = "Power"
  )
...
sim_results %>%
  ...
  labs(
    title = "Average estimate of mu by the true value of mu",
    x = "True Mean",
    y = "Estimated Mean"
  )
```